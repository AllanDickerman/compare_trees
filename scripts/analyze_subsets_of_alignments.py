...
def main():
    ...
    for alignment_file in glob.glob("protein_alignments/*afa"):
        ...
        if m:
            ...
            if args.pgfam_list and (homology_group not in pgfam_set):
                continue
    ...
    if args.dna:
        dna_tree_file = open(f"dna_{args.output_base}_sampleSize{sample_size}.nex", 'w') 
        dna_tree_file.write(f"#NEXUS\n\nBegin Taxa;\n  Dimensions ntax={len(genomeIds)};\n")
        dna_tree_file.write("  TaxLabels "+" ".join(sorted(genomeIds))+";\nEND;\n\nBegin TREES;\n")
    if args.protein:
        protein_tree_file = open(f"protein_{args.output_base}_sampleSize{sample_size}.nex", 'w') 
        protein_tree_file.write(f"#NEXUS\n\nBegin Taxa;\n  Dimensions ntax={len(genomeIds)};\n")
        protein_tree_file.write("  TaxLabels "+" ".join(sorted(genomeIds))+";\nEND;\n\nBegin TREES;\n")
    if args.joint:
        joint_tree_file = open(f"joint_{args.output_base}_sampleSize{sample_size}.nex", 'w') 
        joint_tree_file.write(f"#NEXUS\n\nBegin Taxa;\n  Dimensions ntax={len(genomeIds)};\n")
        joint_tree_file.write("  TaxLabels "+" ".join(sorted(genomeIds))+";\nEND;\n\nBegin TREES;\n")

    homologs_per_sample_file = open(f"homologs_{args.output_base}_sampleSize{sample_size}.txt", "w")

    al_score_file_name = f"alignment_{args.output_base}_sample_scores_sampleSize{sample_size}"
    if args.dna:
    ...
    for sample_pos in range(0, len(dna_list)-sample_size+1, sample_size):
        ...
        sample_label = f"{args.output_base}_{sample_size}genes_{sample_pos:04}"
